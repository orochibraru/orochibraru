...
wizard, then a project with three services — two of them actually deployed as
...
Every service you own, with its status, image and project. Search, filters and
...
A project puts its services on a shared Docker network, so they reach each other
by slug (`http://cache:6379`) without going back out through Traefik.

![A project](images/project.png) ![A project, dark](images/project-dark.png)
